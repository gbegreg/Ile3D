...
type
  ...
  TfPrincipale = class(TForm)
    ...
    dmyNuages: TDummy;
    TextureNuage1: TTextureMaterialSource;
    TextureNuage2: TTextureMaterialSource;
    TextureNuage3: TTextureMaterialSource;
    ...
    TextureDrapeau: TLightMaterialSource;
    TexrureMat: TLightMaterialSource;
    Light2: TLight;
    Layout1: TLayout;
    TrackBar1: TTrackBar;
    palmier1: TModel3D;
    modeleArbre2Mat11: TLightMaterialSource;
    modeleArbre2Mat01: TLightMaterialSource;
    modeleArbre2Mat21: TLightMaterialSource;
    palmier2: TModel3D;
    palmier3: TModel3D;
    palmier4: TModel3D;
    palmier5: TModel3D;
    ...
    procedure v3DMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; var Handled: Boolean);
...
const
  ...
  TailleJoueur = 1;// Taille du joueur
...
begin
  ...
  TTask.Create( procedure
                begin
                  CalcMesh(pDrapeau, Point3D(0,0,0), Point3D(pDrapeau.SubdivisionsWidth, pDrapeau.SubdivisionsHeight, 0) * 0.5 + Point3D(0,0,0) * center, Point3D(0.001, 0.1, 20), pDrapeau.SubdivisionsHeight); // Animtion du drapeau
                end).start;
end;
...
begin
  fPrincipale.Caption := 'Démo île [FPS : '+fps.ToString+']';
  fps := 0;
end;
...
procedure TfPrincipale.v3DMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; var Handled: Boolean);
begin
  vitesse := vitesse + 2/WheelDelta;
  interactionIHM;
end;
...
var
  ...
  taille : integer;
begin
  if dmyNuages.ChildrenCount-1 < 40 then // Création des TPlane pour les nuages
  begin
    s:=TPlane.Create(nil);
    ...
    case random(3) mod 3 of    // Affectation alétoirement d'une des 3 textures de nuage disponibles
      0 : begin
            s.MaterialSource:=textureNuage1;
            s.SetSize(taille,taille/2,0.001);
          end;
      1 : begin
            s.MaterialSource:=textureNuage2;       // On lui affecte la texture
  ...
  begin
      ...
      s.position.x := s.position.x + 30 / ( -s.Position.Y); // On le décalle sur l'axe X (d'ouest en est) en fonction de son altitude (les nuages les plus bas se déplaceront plus rapidement que ceux d'altitute)
...
begin
  ...
  begin
    if  (mSol.Children[i] is TModel3D) or (mSol.Children[i] is TCylinder) or (mSol.Children[i] is TProxyObject) then
    begin
      ...
      distanceMinimum := (SizeOf3D(unObjet3D) + SizeOf3D(dmyProchainePosition)) ;//* 0.5; // distanceMinimum : on divise par 2 car le centre de l'objet est la moitié de la taille de l'élément sur les 3 composantes X, Y, Z
